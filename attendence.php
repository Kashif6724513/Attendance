...
if (isset($_POST['check_in'])) {
    if (!$attendance) {
        ...
        if (mysqli_query($conn, $query)) {
            echo "<script>alert('Checked in successfully!'); window.location.href='admin_dashboard.php';</script>";
        } else {
            echo "Error: " . mysqli_error($conn);
        }
    } else {
        echo "<script>alert('You have already checked in today!'); window.location.href='admin_dashboard.php';</script>";
    }
}
...
if (isset($_POST['start_break'])) {
    if ($attendance && !$attendance['break_in']) {
        ...
        if (mysqli_query($conn, $updateQuery)) {
            echo "<script>alert('Break started successfully!'); window.location.href='admin_dashboard.php';</script>";
        } else {
            echo "Error: " . mysqli_error($conn);
        }
    } else {
        echo "<script>alert('You have already started a break!'); window.location.href='admin_dashboard.php';</script>";
    }
}
...
if (isset($_POST['end_break'])) {
    if ($attendance && $attendance['break_in'] && !$attendance['break_out']) {
        ...
        if (mysqli_query($conn, $updateQuery)) {
            echo "<script>alert('Break ended successfully!'); window.location.href='admin_dashboard.php';</script>";
        } else {
            echo "Error: " . mysqli_error($conn);
        }
    } else {
        echo "<script>alert('You have not started a break or already ended it!'); window.location.href='admin_dashboard.php';</script>";
    }
}
...
if (isset($_POST['check_out'])) {
    if ($attendance && !$attendance['checkout']) {
        ...
        if (mysqli_query($conn, $updateQuery)) {
            echo "<script>alert('Checked out successfully!'); window.location.href='admin_dashboard.php';</script>";
        } else {
            echo "Error: " . mysqli_error($conn);
        }
    } else {
        echo "<script>alert('You have already checked out!'); window.location.href='admin_dashboard.php';</script>";
    }
}
?>
...
<nav class="navbar navbar-expand-lg bg-body-tertiary">
    ...
    <h1 class="text-success">Attendence Form</h1>
        <div class="card p-4 shadow-lg" style="margin-top: 80px;">
            ...
            <form action="" name="form" method="post">
                ...
                <?php endif; ?>
                <button class="btn btn-primary"><a href="admin_dashboard.php" class="text-decoration-none text-white">Back</a></button>
            </form>
        </div>
    </div>
